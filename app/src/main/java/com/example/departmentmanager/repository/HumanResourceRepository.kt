...
import com.example.departmentmanager.network.END_POINT_GET_ALL_DEPARTMENT
import com.google.gson.Gson
import kotlinx.coroutines.*
import javax.inject.Inject

class HumanResourceRepository @Inject constructor(private val api: Api) {
    fun getAllDepartment() : LiveData<Result<List<Department>>>{
        val url = END_POINT_GET_ALL_DEPARTMENT
        ...
        CoroutineScope(Dispatchers.IO + coroutineExceptionHandler).launch {
            ...
            withContext(Dispatchers.Main) {
                if (request.isSuccessful) {
                    ...
                    departmentsLiveData.postValue(Result.Failures(status, request.code(), request.message()))
                }
            }
        }
        ...
        CoroutineScope(Dispatchers.IO + coroutineExceptionHandler).launch {
            ...
            withContext(Dispatchers.Main) {
                if (request.isSuccessful) {
                    ...
                    val strErr = request.errorBody()?.string()
                    val status = Gson().fromJson(strErr, ErrorResponse::class.java)
                    createdEmployeeLiveData.postValue(Result.Failures(status, request.code(), request.message()))
                }
            }
        }
    ...
    fun getEmployeeOfDepartment(idDepartment: Int?) : LiveData<Result<List<Employee>>>{
        ...
        CoroutineScope(Dispatchers.IO + coroutineExceptionHandler).launch {
            ...
            val request = api.getEmployeeOfDepartment(url = url, idDepartment = idDepartment)
            withContext(Dispatchers.Main) {
                if (request.isSuccessful) {
                    ...
                    val strErr = request.errorBody()?.string()
                    val status = Gson().fromJson(strErr, ErrorResponse::class.java)
                    employeesOfDepartment.postValue(Result.Failures(status, request.code(), request.message()))
                }
            }
        }
        ...
        CoroutineScope(Dispatchers.IO + coroutineExceptionHandler).launch {
            ...
            withContext(Dispatchers.Main) {
                if (request.isSuccessful) {
                    ...
                    val strErr = request.errorBody()?.string()
                    val status = Gson().fromJson(strErr, ErrorResponse::class.java)
                    updatedEmployeeLiveData.postValue(Result.Failures(status, request.code(), request.message()))
                }
            }
        }
